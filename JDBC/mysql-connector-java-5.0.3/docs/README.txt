...
1.1.4. Retrieving AUTO_INCREMENT Column Values
   ...
   The following code snippets demonstrate the three different ways
   to retrieve AUTO_INCREMENT values. First, we demonstrate the use
   of the new JDBC-3.0 method getGeneratedKeys() which is now the
   preferred method to use if you need to retrieve AUTO_INCREMENT
   keys and have access to JDBC-3.0. The second example shows how you
   can retrieve the same value using a standard SELECT
   LAST_INSERT_ID() query. The final example shows how updatable
   result sets can retrieve the AUTO_INCREMENT value when using the
   insertRow() method.
...
()': "
   ...
   Example 10. Retrieving AUTO_INCREMENT Column Values in Updatable
   ResultSets
   Statement stmt = null;
   ResultSet rs = null;
...
1.2.1.1. Java Versions Supported
   ...
   newer to compile (but not run). MySQL Connector/J does not support
   JDK-1.1.x or JDK-1.0.x
   ...
   class verifier is turned off (-Xverify:none), as the class
   verifier will try to load the class definition for
...
1.2.1.2. MySQL Server Version Guidelines
   ...
   MySQL Connector/J supports all known MySQL server versions. Some
   features (foreign keys, updatable result sets) require more recent
   versions of MySQL to operate.

   When connecting to MySQL server version 4.1 or newer, it is best
   ...
   version 4.1 or newer, and implements Unicode characters and the
   new authorization mechanism, Connector/J 3.0 will not be updated
   to support new features in current and future server versions.
...
1.2.1.3. Installing the Driver and Configuring the CLASSPATH
   ...
   .jar archive named "mysql-connector-java-[version]-bin.jar", and
   starting with Connector/J 3.1.8 a "debug" build of the driver in a
   file named "mysql-connector-java-[version]-bin-g.jar".
   ...
   instructed to do so when reporting a problem or bug to MySQL AB,
   ...
   also depends on the Aspect/J runtime library, which is located in
   the src/lib/aspectjrt.jar file that comes with the Connector/J
   distribution.
...
$ setenv CLASSPATH /path/to/mysql-connector-java-[version]-bin.jar:$C
LASSPATH

   The above command can be added to the appropriate startup file for
   the login shell to make MySQL Connector/J available to all Java
   applications.
   ...
   application servers, see Section A.4, "Using Connector/J with J2EE
...
1.2.2.1. Upgrading from MySQL Connector/J 3.0 to 3.1
   ...
   configuration properties.
     * Unicode Character Sets --- See the next section, as well as
       [WARNING: missing xref target (id=charset)] for information on
       this new feature of MySQL. If you have something
     ...
     * Datetimes with all-zero components ('0000-00-00 ...') ---
       ...
       property. The allowable values are: 'exception' (the default),
       which throws an SQLException with an SQLState of 'S1009',
       'convertToNull', which returns NULL instead of the date, and
       ...
       (the default value is 'false') so that you can get retrieve
       the unaltered all-zero value as a String. It should be noted
       that this also precludes using any time zone conversions,
       therefore the driver will not allow you to enable
       noDatetimeStringSync and useTimezone at the same time.
     * New SQLState Codes --- Connector/J 3.1 uses SQL:1999 SQLState
       ...
       than MySQL-4.1.0 (the oldest version to return SQLStates as
     ...
     * Calling ResultSet.getString() on a BLOB column will now return
       the address of the byte[] array that represents it, instead of
       a String representation of the BLOB. BLOBs have no character
       set, so they can't be converted to java.lang.Strings without
       data loss or corruption.
       To store strings in MySQL with LOB behavior, use one of the
       TEXT types, which the driver will treat as a java.sql.Clob.
     * Starting with Connector/J 3.1.8 a "debug" build of the driver
       in a file named "mysql-connector-java-[version]-bin-g.jar" is
       shipped alongside the normal "binary" jar file that is named
...
1.2.2.2. JDBC-Specific Issues When Upgrading to MySQL Server 4.1 or
Newer
...
Caution
   ...
   follow these steps:
    1. At the time of this writing, there are three active branches
       of Connector/J: branch_3_0, branch_3_1 and branch_5_0. Check
       out the latest code from the branch that you want with the
       following command (replacing [major] and [minor] with
       appropriate version numbers):
shell> svn co http://svn.mysql.com/svnpublic/connector-j/branches/bra
nch_[major]_[minor]/connector-j
...
shell> ant --projecthelp
    4. A newly created .jar file containing the JDBC driver will be
       ...
       instructions in Section A.2.1.3, "Installing the Driver and
       Configuring the CLASSPATH."

1.3. JDBC Reference

1.3.1. Driver/Datasource Class Names, URL Syntax and Configuration
Properties for Connector/J
   ...
   backward-compatible with MM.MySQL. You should use this class name
   when registering the driver, or when otherwise configuring
...
jdbc:mysql://[host][,failoverhost...][:port]/[database][?propertyName
...
jdbc:mysql://[host:port],[host:port].../[database][?propertyName1][=p
ropertyValue1][&propertyName2][=propertyValue2]...
   ...
   fail-over to any number of "slave" hosts and still perform
   ...
   driver determines that the first host has become available again.

   If the URL property "autoReconnect" is true: Failover happens when
   the driver determines that the connection has failed (before every
   query), and falls back to the first host when it determines that
   the host has become available again (after
...
Note
   ...
   'Configurations' of the documentation. No 3.1.5
   ...
   of WAIT_TIMEOUT No false 3.1.0
   ...
   attempting a connection No 3.1.4
   useCompression Use zlib compression when communicating with the
   server (true/false)? Defaults to 'false'. No false 3.0.17
   ...
   don'thandle SQLExceptions properly, and is only designed to be
   ...
   reconnect to the master. Defaults to 50. No 50 3.0.2
   secondsBeforeRetryMaster How long should the driver wait, when
   failed over, before attempting to reconnect to the master server?
   Whichever condition is met first, 'queriesBeforeRetryMaster' or
   'secondsBeforeRetryMaster' will cause an attempt to be made to
   reconnect to the master. Time in seconds, defaults to 30 No 30
   3.0.2
   enableDeprecatedAutoreconnect Auto-reconnect functionality is
   deprecated starting with version 3.2, and will be removed in
   version 3.3. Set this property to 'true' to disable the check for
   the feature being configured. No false 3.2.1
   ...
   when possible? (defaults to 'false') No false 3.0.1

   Performance Extensions. 
   Property Name Definition Required? Default Value Since Version
   metadataCacheSize The number of queries to cacheResultSetMetadata
   for if cacheResultSetMetaData is set to 'true' (default 50) No 50
   3.1.1
   prepStmtCacheSize If prepared statement caching is enabled, how
   many prepared statements should be cached? No 25 3.0.10
   prepStmtCacheSqlLimit If prepared statement caching is enabled,
   what's the largest SQL the driver will cache the parsing for? No
   256 3.0.10
   ...
   to retrieve rows? No false 5.0.0
   ...
   statements themselves? No false 3.0.10
   ...
   default 'false') No false 3.1.1
   cacheServerConfiguration Should the driver cache the results of
   'SHOW VARIABLES' and 'SHOW COLLATION' on a per-URL basis? No false
   3.1.5
   defaultFetchSize The driver will call setFetchSize(n) with this
   value on all newly-created Statements No 0 3.1.9
   ...
   memory efficient for some applications. No false 3.1.7
   dynamicCalendars Should the driver retrieve the default calendar
   when required, or cache it per connection/session? No false 3.1.5
   ...
   Connection.setAutoCommit(boolean)? No false 3.1.3
   ...
   for getBinaryInputStream? No 1048576 3.2.1
   ...
   determine the optimium number of parameters per batch and you
   might receive anan error from the driver that the resultant packet
   is too large. Statement.getGeneratedKeys() for these rewritten
   statements only works when the entire batch includes INSERT
   statements. No false 3.1.13
   useFastIntParsing Use internal String->Integer conversion routines
   to avoid excessive object creation? No true 3.1.4
   useJvmCharsetConverters Always use the character encoding routines
   built into the JVM, rather than using lookup tables for
   single-byte character sets? (The default of "true" for this is
   appropriate for newer JVMs No true 5.0.1
   ...
   database? No false 3.1.7
   useReadAheadInput Use newer, optimized non-blocking, buffered
   input stream when reading from the server? No true 3.1.5

   Debuging/Profiling. 
   Property Name Definition Required? Default Value Since Version
   logger The name of a class that implements
   'com.mysql.jdbc.log.Log' that will be used to log messages
   to.(default is 'com.mysql.jdbc.log.StandardLogger', which logs to
   STDERR) No com.mysql.jdbc.log.StandardLogger 3.1.1
   profileSQL Trace queries and their execution/fetch times to the
   configured logger (true/false) defaults to 'false' No false 3.1.0
   reportMetricsIntervalMillis If 'gatherPerfMetrics' is enabled, how
   often should they be logged (in ms)? No 30000 3.1.2
   maxQuerySizeToLog Controls the maximum length/size of a query that
   will get logged when profiling or tracing No 2048 3.1.3
   ...
   should a query (in ms) before it is logged as 'slow'? No 2000
   3.1.2
   ...
   results to the configured log at a WARN level? No false 3.1.2
   logSlowQueries Should queries that take longer than
   'slowQueryThresholdMillis' be logged? No false 3.1.2
   traceProtocol Should trace-level network protocol be logged? No
   false 3.1.2
   ...
   methods? No false 3.1.12
   autoDeserialize Should the driver automatically detect and
   de-serialize objects stored in BLOB fields? No false 3.1.5
   ...
   configured connection characterEncoding No 5.0.0
   ...
   before all the data has been read from the server. No false 3.0.9
   ...
   (defaults to 'true'). No true 3.0.3
   ...
   to create databases. No false 3.1.9
   emptyStringsConvertToZero Should the driver allow conversions from
   empty string fields to numeric values of '0'? No true 3.1.8
   ...
   with client-side emulated versions? No true 3.1.7
   ignoreNonTxTables Ignore non-transactional table warning for
   rollback? (defaults to 'false'). No false 3.0.9
   ...
   supports warnings(MySQL 4.1.0 and newer)? No true 3.1.2
   maxRows The maximum number of rows to return (0, the default means
   return all rows). No -1 all versions
   ...
   the server timezone if 'useTimezone'='true' No false 5.0.0
   ...
   from earlier versions of the driver) No true 3.1.8
   ...
   departure from the specification) No true 3.1.8
   ...
   application being OpenOffice)? No false 3.1.12
   ...
   support "XA START ... JOIN" after "XA END" has been called No
   false 5.0.1
   processEscapeCodesForPrepStmts Should the driver process escape
   codes in queries that are prepared? No true 3.1.12
   ...
   been called. This is not JDBC-compliant after JDBC-4.0. No false
   3.1.11
   rollbackOnPooledClose Should the driver issue a rollback() when
   the logical connection in a pool is closed? No true 3.0.15
   runningCTS13 Enables workarounds for bugs in Sun's JDBC compliance
   testsuite version 1.3 No false 3.1.7
   serverTimezone Override detection/mapping of timezone. Used when
   timezone from server doesn't map to Java timezone No 3.0.2
   ...
   TINYINT(1) when creating tables)? No true 3.0.16
   ...
   legacy behavior, "true" leads to more JDBC-compliant behavior. No
   false 3.1.12
   ...
   used by DatabaseMetaData? No false 5.0.0
   ...
   'round' and 'convertToNull'. No exception 3.1.4
...
1.3.2. JDBC API Implementation Notes
   ...
   Connector/J.
     * Blob
       ...
       your JDBC URL. You must then use a column alias with the value
       of the column set to the actual name of the Blob column in the
       SELECT that you write to retrieve the Blob. The SELECT must
       also reference only one table, the table must have a primary
       key, and the SELECT must cover all columns that make up the
       primary key. The driver will then delay loading the actual
       Blob data until you retrieve the Blob and call retrieval
       methods (getInputStream(), getBytes(), and so forth) on it.
     * CallableStatement
     ...
     * Connection
       ...
       not "ping" the server to determine if it is alive. In
     ...
     * PreparedStatement
       ...
       parameter changed to a non-"large" parameter, it is necessary
       to call clearParameters() and set all parameters again. The
       reason for this is as follows:
          ...
          + If a parameter changes from "large" to non-"large," the
            ...
            take the place of the prior "large" value. This removes
       ...
       to a non-"large" one, you must call clearParameters() and set
       all parameters of the prepared statement again before it can
       be re-executed.
     * ResultSet
...
1.3.3. Java, JDBC and MySQL Types
   ...
   so by using the property "jdbcCompliantTruncation" and setting it
   to "false".
   ...
   Note: round-off, overflow or loss of precision may occur if you
   choose a Java numeric data type that has less precision or
   capacity than the MySQL data type you are converting to/from.

   The ResultSet.getObject() method uses the following type
   conversions between MySQL and Java types, following the JDBC
   ...
   "tinyInt1isBit" is set to "true" (the default) and the storage
   size is "1", or java.lang.Integer if not.
   BOOL , BOOLEAN See TINYINT, above as these are aliases for
   TINYINT(1), currently.
   SMALLINT[(M)] [UNSIGNED] java.lang.Integer (regardless if UNSIGNED
   or not)
   ...
   TIME java.sql.Time
   YEAR[(2|4)] java.sql.Date (with the date set two January 1st, at
   midnight)
   CHAR(M) java.lang.String (unless the character set for the column
...
1.3.4. Using Character Sets and Unicode
   ...
   server versions 4.1.0 and newer. For more information, see
   [WARNING: missing xref target (id=charset-server)]
...
1.3.5. Connecting Securely Using SSL
   ...
   For SSL Support to work, you must have the following:
     ...
     * A MySQL server that supports SSL and has been compiled and
       ...
       http://www.mysql.com/doc/en/secure-connections.html
   ...
   in the 'SSL' subdirectory of the MySQL source distribution. This
   is what SSL will use to determine if you are communicating with a
   secure MySQL server.
   ...
   It's located in the 'bin' subdirectory of your JDK or JRE):
shell> keytool -import -alias mysqlServerCACert -file cacert.pem -key
store truststore


   Keytool will respond with the following information:
Enter keystore password:  *********
Owner: EMAILADDRESS=walrus@example.com, CN=Walrus, O=MySQL AB, L=Oren
burg, ST=Some
-State, C=RU
Issuer: EMAILADDRESS=walrus@example.com, CN=Walrus, O=MySQL AB, L=Ore
nburg, ST=Som
e-State, C=RU
Serial number: 0
Valid from: Fri Aug 02 16:55:53 CDT 2002 until: Sat Aug 02 16:55:53 C
DT 2003
...
Certificate was added to keystore
 ...
 shell> keytool -genkey -keyalg rsa -alias mysqlClientCertificate -ke
ystore keystore
...
-Djavax.net.ssl.trustStorePassword=*********
   ...
   to your URL, or by setting the property 'useSSL' to 'true' in the
 ...
 *** ClientHello, v3.1
 ...
 54, 244, 0, 168, 55, 103, 215, 64, 16, 138, 225, 190, 132, 153, 2, 2
17, 219, 239, 202, 19, 121, 78 }
 Session ID:  {}
 ...
 *** ServerHello, v3.1
 RandomCookie:  GMT: 1018577560 bytes = { 116, 50, 4, 103, 25, 100, 5
8, 202, 79, 185, 178, 100, 215, 66, 254, 21, 83, 187, 190, 42, 170, 3
, 132, 110, 82, 148, 160, 92 }
 Session ID:  {163, 227, 84, 53, 81, 127, 252, 254, 178, 179, 68, 63,
 182, 158, 30, 11, 150, 79, 170, 76, 255, 92, 15, 226, 24, 17, 177, 2
19, 158, 177, 187, 143}
 Cipher Suite:  { 0, 5 }
 ...
 ** SSL_RSA_WITH_RC4_128_SHA
 ...
 0000: 02 00 00 46 03 01 3D B6   43 98 74 32 04 67 19 64  ...F..=.C.t
2.g.d
 0010: 3A CA 4F B9 B2 64 D7 42   FE 15 53 BB BE 2A AA 03  :.O..d.B..S
..*..
 0020: 84 6E 52 94 A0 5C 20 A3   E3 54 35 51 7F FC FE B2  .nR..\ ..T5
Q....
...
1.3.6. Using Master/Slave Replication with ReplicationConnection
   ...
   connection is "sticky" to a slave unless that slave is removed
   from service). If you have a write transaction, or if you have a
...
test",
        ...
        //
        // Now, do a query from a slave, the driver automatically pic
ks one
        // from the list
        //
...
s
        ...
        DataSource ds = (DataSource)ctx.lookup("java:comp/env/jdbc/My
SQLDB");
...
e
   ...
   depends." Although the optimal size depends on anticipated load
...
1.4.2. Using Connector/J with Tomcat
   ...
   http://jakarta.apache.org/tomcat/tomcat-5.0-doc/jndi-datasource-ex
   amples-howto.html which is current at the time this document was
   written.
...
1.4.3. Using Connector/J with JBoss
   ...
   configuration (which is usually called "default"). Then, in the
   same configuration directory, in the subdirectory named "deploy,"
...
.ExtendedMysqlExceptionSorter</exception-sorter-class-name>
        <valid-connection-checker-class-name>com.mysql.jdbc.integrati
on.jboss.MysqlValidConnectionChecker</valid-connection-checker-class-
name>
...
1.5.1. Common Problems and Solutions
   ...
   Questions
     * [1]1.5.1.1: When I try to connect to the database with MySQL
...
VendorError: 0
       ...
       command-line client.
     * [2]1.5.1.2: My application throws an SQLException 'No Suitable
       Driver'. Why is this happening? 
     * [3]1.5.1.3: I'm trying to use MySQL Connector/J in an applet
...
SQLState: 08S01
VendorError: 0
     * [4]1.5.1.4: I have a servlet/application that works fine for a
       day, and then stops working overnight 
     * [5]1.5.1.5: I'm trying to use JDBC-2.0 updatable result sets,
       and I get an exception saying my result set is not updatable. 

   Questions and Answers

   1.5.1.1: When I try to connect to the database with MySQL
   Connector/J, I get the following exception: 
SQLException: Server configuration denies access to data source
SQLState: 08001
VendorError: 0
   ...
   You must add grants to allow this to happen. The following is an
   example of how to do this (but not the most secure).

   From the mysql command-line client, logged in as a user that can
   grant privileges, issue the following command:
GRANT ALL PRIVILEGES ON [dbname].* to
                '[user]'@'[hostname]' identified by
                '[password]'

   replacing [dbname] with the name of your database, [user] with the
   user name, [hostname] with the host that MySQL Connector/J will be
   connecting from, and [password] with the password you want to use.
   Be aware that RedHat Linux is broken with respect to the hostname
   portion for the case when you are connecting from localhost. You
   need to use "localhost.localdomain" for the [hostname] value in
   this case. Follow this by issuing the "FLUSH PRIVILEGES" command.
...
Warning
   ...
   If you don't understand what the 'GRANT' command does, or how it
   works, you should read and understand the 'General Security Issues
   and the MySQL Access Privilege System'
   (http://www.mysql.com/doc/en/Privilege_system.html) section of the
   MySQL manual before attempting to change privileges.
   ...
   1.5.1.2: My application throws an SQLException 'No Suitable
   Driver'. Why is this happening? 

   One of two things are happening. Either the driver is not in your
   CLASSPATH or your URL format is incorrect (see the Section A.2,
   "Installing Connector/J" section.).

   1.5.1.3: I'm trying to use MySQL Connector/J in an applet or
...
VendorError: 0
   ...
   1.5.1.4: I have a servlet/application that works fine for a day,
   ...
   use the "autoReconnect" parameter (see Section A.3.1,
...
he
                ...
                // transaction really depends on your application log
ic,
                // whether or not you're using autocommit (in this ca
se
                // not), and whether you're using transacational stor
age
                // engines
                //
                // For this example, we'll assume that it's _not_ saf
e
                // to retry the entire transaction, so we set retry c
ount
                // to 0 at this point
                //
                // If you were using exclusively transaction-safe tab
les,
                // or your application could recover from a connectio
n going
                // bad in the middle of an operation, then you would
not
                // touch 'retryCount' here, and just let the loop rep
eat
                // until retryCount == 0.
                //
                retryCount = 0;
...
.
                //
                // Only retry if the error was due to a stale connect
ion,
                ...
                if ("08S01".equals(sqlState) || "40001".equals(sqlSta
te)) {
                    retryCount--;
                } else {
                    retryCount = 0;
...
. .
   ...
   1.5.1.5: I'm trying to use JDBC-2.0 updatable result sets, and I
...
1.5.2. How to Report Connector/J Bugs or Problems
   ...
   email to security_at_@mysql.com (mailto:security_at_mysql.com).
...
1.6.1. Changes in MySQL Connector/J 5.0.2-beta (11 July 2006)

     * Fixed can't use XAConnection for local transactions when no
       global transaction is in progress. (fixes
     ...
     * Added connection/datasource property
       ...
       "XA START ... JOIN" after "XA END" has been called, and is
       also a workaround for transaction managers that don't maintain
       thread affinity for a global transaction (most either always
       maintain thread affinity, or have it as a configuration
       option).
...
1.6.2. Changes in MySQL Connector/J 5.0.1-beta (Not Released)

   Not released due to a packaging error

1.6.3. Changes in MySQL Connector/J 5.0.0-beta (22 December 2005)
...
1.6.4. Changes in MySQL Connector/J 3.1.14 (not yet released)
     ...
     * Fixed memory leak with profileSQL=true. (Fixes
       Bug#16987: http://bugs.mysql.com/16987)
     * - - Connection fails to localhost when using timeout and IPv6
       is configured. (Fixes Bug#19726: http://bugs.mysql.com/19726)
     ...
     * Fixed ResultSet.getShort() for UNSIGNED TINYINT returns
       ...
       (Fixes Bug#20306: http://bugs.mysql.com/20306)
     * Fixed can't pool server-side prepared statements, exception
       raised when re-using them. (Fixes
       Bug#20687: http://bugs.mysql.com/20687 -

1.6.5. Changes in MySQL Connector/J 3.1.13 (26 May 2006)
     ...
     * Improved performance of retrieving BigDecimal, Time, Timestamp
       ...
       Fixes Bug#18496: http://bugs.mysql.com/18496 for BigDecimals.
...
1.6.6. Changes in MySQL Connector/J 3.1.12 (30 November 2005)
     ...
     * Process escape tokens in Connection.prepareStatement(...).
       (Bug#15141: http://bugs.mysql.com/15141) You can disable this
...
1.6.7. Changes in MySQL Connector/J 3.1.11-stable (07 October 2005)
     ...
     * URL configuration parameters don't allow `&' or `=' in their
       ...
       tml). (Bug#13453: http://bugs.mysql.com/13453)
...
1.6.8. Changes in MySQL Connector/J 3.1.10-stable (23 June 2005)
...
1.6.9. Changes in MySQL Connector/J 3.1.9-stable (22 June 2005)
...
1.6.10. Changes in MySQL Connector/J 3.1.8-stable (14 April 2005)
     ...
     * DATE_FORMAT() queries returned as BLOBs from getObject().
...
1.6.11. Changes in MySQL Connector/J 3.1.7-stable (18 February 2005)
...
1.6.12. Changes in MySQL Connector/J 3.1.6-stable (23 December 2004)
...
1.6.13. Changes in MySQL Connector/J 3.1.5-gamma (02 December 2004)
...
1.6.14. Changes in MySQL Connector/J 3.1.4-beta (04 September 2004)
...
1.6.15. Changes in MySQL Connector/J 3.1.3-beta (07 July 2004)
...
1.6.16. Changes in MySQL Connector/J 3.1.2-alpha (09 June 2004)
...
1.6.17. Changes in MySQL Connector/J 3.1.1-alpha (14 February 2004)
...
1.6.18. Changes in MySQL Connector/J 3.1.0-alpha (18 February 2003)
...
1.6.19. Changes in MySQL Connector/J 3.0.17-ga (23 June 2005)
     ...
     * Adding CP943 to aliases for sjis.
       (Bug#6549: http://bugs.mysql.com/6549, fixed while fixing
...
1.6.20. Changes in MySQL Connector/J 3.0.16-ga (15 November 2004)
...
1.6.21. Changes in MySQL Connector/J 3.0.15-production (04 September
2004)
     ...
     * Failover for autoReconnect not using port numbers for any
       hosts, and not retrying all hosts. (Warning: This required a
       change to the SocketFactory connect() method signature, which
       is now public Socket connect(String host, int portNumber,
       Properties props); therefore, any third-party socket factories
       will have to be changed to support this signature.
...
1.6.22. Changes in MySQL Connector/J 3.0.14-production (28 May 2004)

     * Fixed URL parsing error.

1.6.23. Changes in MySQL Connector/J 3.0.13-production (27 May 2004)
...
1.6.24. Changes in MySQL Connector/J 3.0.12-production (18 May 2004)
     ...
     * Allow java.util.Date to be sent in as parameter to
       ...
       maintain full precision. (Bug#103: http://bugs.mysql.com/103).
...
1.6.25. Changes in MySQL Connector/J 3.0.11-stable (19 February 2004)
...
1.6.26. Changes in MySQL Connector/J 3.0.10-stable (13 January 2004)
...
1.6.27. Changes in MySQL Connector/J 3.0.9-stable (07 October 2003)
     ...
     * Support InnoDB contraint names when extracting foreign key
       ...
       Parwinder Sekhon). (Bug#517: http://bugs.mysql.com/517,
...
1.6.28. Changes in MySQL Connector/J 3.0.8-stable (23 May 2003)
...
1.6.29. Changes in MySQL Connector/J 3.0.7-stable (08 April 2003)
...
1.6.30. Changes in MySQL Connector/J 3.0.6-stable (18 February 2003)
...
1.6.31. Changes in MySQL Connector/J 3.0.5-gamma (22 January 2003)
...
1.6.32. Changes in MySQL Connector/J 3.0.4-gamma (06 January 2003)
...
1.6.33. Changes in MySQL Connector/J 3.0.3-dev (17 December 2002)
...
1.6.34. Changes in MySQL Connector/J 3.0.2-dev (08 November 2002)
...
1.6.35. Changes in MySQL Connector/J 3.0.1-dev (21 September 2002)
...
1.6.36. Changes in MySQL Connector/J 3.0.0-dev (31 July 2002)
...
1.6.37. Changes in MySQL Connector/J 2.0.14 (16 May 2002)
...
1.6.38. Changes in MySQL Connector/J 2.0.13 (24 April 2002)
...
1.6.39. Changes in MySQL Connector/J 2.0.12 (07 April 2002)
...
1.6.40. Changes in MySQL Connector/J 2.0.11 (27 January 2002)
...
1.6.41. Changes in MySQL Connector/J 2.0.10 (24 January 2002)
...
1.6.42. Changes in MySQL Connector/J 2.0.9 (13 January 2002)
...
1.6.43. Changes in MySQL Connector/J 2.0.8 (25 November 2001)
...
1.6.44. Changes in MySQL Connector/J 2.0.7 (24 October 2001)
...
1.6.45. Changes in MySQL Connector/J 2.0.6 (16 June 2001)

     * Fixed PreparedStatement parameter checking.
     * Fixed case-sensitive column names in ResultSet.java.

1.6.46. Changes in MySQL Connector/J 2.0.5 (13 June 2001)
...
1.6.47. Changes in MySQL Connector/J 2.0.3 (03 December 2000)
...
1.6.48. Changes in MySQL Connector/J 2.0.1 (06 April 2000)
...
1.6.49. Changes in MySQL Connector/J 2.0.0pre5 (21 February 2000)

     * Fixed Bad Handshake problem.

1.6.50. Changes in MySQL Connector/J 2.0.0pre4 (10 January 2000)
...
1.6.51. Changes in MySQL Connector/J 2.0.0pre (17 August 1999)
...
1.6.52. Changes in MySQL Connector/J 1.2b (04 July 1999)
...
1.6.53. Changes in MySQL Connector/J 1.2a (14 April 1999)
...
1.6.54. Changes in MySQL Connector/J 1.1i (24 March 1999)

     * Fixed Timestamps for PreparedStatements
     * Fixed null pointer exceptions in RSMD and RS
     * Re-compiled with jikes for valid class files (thanks ms!)

1.6.55. Changes in MySQL Connector/J 1.1h (08 March 1999)
...
1.6.56. Changes in MySQL Connector/J 1.1g (19 February 1999)
...
1.6.57. Changes in MySQL Connector/J 1.1f (31 December 1998)
...
1.6.58. Changes in MySQL Connector/J 1.1b (03 November 1998)
...
1.6.59. Changes in MySQL Connector/J 1.1 (02 September 1998)
...
1.6.60. Changes in MySQL Connector/J 1.0 (24 August 1998)
...
1.6.61. Changes in MySQL Connector/J 0.9d (04 August 1998)
...
1.6.62. Changes in MySQL Connector/J 0.9 (28 July 1998)
...
1.6.63. Changes in MySQL Connector/J 0.8 (06 July 1998)
...
1.6.64. Changes in MySQL Connector/J 0.7 (01 July 1998)
...
1.6.65. Changes in MySQL Connector/J 0.6 (21 May 1998)
...
References
   ...
   1. file://localhost/home/mysqldoc/docs-build/mysqldoc/refman-5.0/connector-j-nolink.html#qandaitem-1-5-1-1
   2. file://localhost/home/mysqldoc/docs-build/mysqldoc/refman-5.0/connector-j-nolink.html#qandaitem-1-5-1-2
   3. file://localhost/home/mysqldoc/docs-build/mysqldoc/refman-5.0/connector-j-nolink.html#qandaitem-1-5-1-3
   4. file://localhost/home/mysqldoc/docs-build/mysqldoc/refman-5.0/connector-j-nolink.html#qandaitem-1-5-1-4
   5. file://localhost/home/mysqldoc/docs-build/mysqldoc/refman-5.0/connector-j-nolink.html#qandaitem-1-5-1-5
